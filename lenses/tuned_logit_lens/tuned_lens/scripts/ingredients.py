...
class Distributed:
    ...
    def dataloader(
        ...
        Prefer the legacy torchdata DataLoader2 path when available, but fall
        back to the standard PyTorch DataLoader for newer torchdata releases
        where `dataloader2` and `datapipes` were removed.
        """
        if dataloader2 is not None and datapipes is not None:
        ...
        if self.world_size > 1:
            raise RuntimeError(
                "This tuned-lens fallback loader only supports single-process training. "
                "Install an older torchdata with dataloader2 support for distributed runs."
            )
