...
class JacobianLens:
    ...
    def load(cls, path: str) -> JacobianLens:
        ...
        Also accepts a fit-progress checkpoint from :func:`jlens.fitting.fit`
        when it contains at least one completed prompt; the stored
        ``jacobian_sum`` is converted into the corresponding mean lens.
        """
        ...
        if fit_keys <= set(checkpoint):
            n_done = checkpoint["n_done"]
            if n_done <= 0:
                raise ValueError(
                    f"{path} is a fit() checkpoint with n_done={n_done}; "
                    "it does not contain a completed Jacobian lens yet"
                )
            jacobians = {
                layer: checkpoint["jacobian_sum"][layer].float() / n_done
                for layer in checkpoint["source_layers"]
            }
            d_model = next(iter(jacobians.values())).shape[0]
            return cls(jacobians=jacobians, n_prompts=n_done, d_model=d_model)

        raise ValueError(
            f"{path} is not a JacobianLens file "
            f"(found keys {sorted(checkpoint)!r})"
        )
