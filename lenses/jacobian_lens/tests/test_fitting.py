...
def test_load_fit_checkpoint_as_lens(tmp_path):
    """A fit-progress checkpoint with completed prompts can be loaded directly."""
    model = TinyDecoder(n_layers=4, d_model=8)
    prompts = ["abcdefghij " * 5, "klmnopqrst " * 5, "uvwxyzabcd " * 5]
    checkpoint = str(tmp_path / "ckpt.pt")
    ...
    loaded = JacobianLens.load(checkpoint)

    assert loaded.n_prompts == fitted.n_prompts
    assert loaded.source_layers == fitted.source_layers
    assert loaded.d_model == fitted.d_model
    for layer in fitted.source_layers:
        torch.testing.assert_close(loaded.jacobians[layer], fitted.jacobians[layer])
